...
% NOTE: dropping the \parbox here (i.e. inputting mycaptionparlabel-html.tex)
% breaks latex2html caption-matching: figures lose their numbers ("Figure:"
% instead of "Figure 2.9:"), and cross-refs go stale.  So we keep the parbox
% version; the figure layout itself (below) forces the caption onto its own
% line so it appears under the figure rather than to its right.
...
% Bare \caption (instead of \mycaptionparlabel) makes latex2html render the
% caption as a real <CAPTION ALIGN="BOTTOM"><STRONG>Figure N:</STRONG>...
% </CAPTION> element instead of a standalone parbox-image (whose ALT text has
% no "Figure N:" prefix).  \protect\small matches the .aux entry written by
% the LaTeX-build counterpart so caption-matching finds a number.
\newcommand{\myFigure}[2]{
	\begin{figure}[htbp]
	\includegraphics{eps/#1} % the figure
        \centering
	\caption{\protect\small #2}\label{fig:#1}
	\end{figure}
}
...
%	\resizebox{#2}{!}{\includegraphics{eps/#1}}
	...
	\caption{\protect\small #3}\label{fig:#1}
	\end{figure}
}
...
%kludge:
	...
	\caption{\protect\small #4 --- [FIXME: Cannot do both rotation and scaling in latex2html graphicx support]}\label{fig:#1}
	\end{figure}
}
...
%kludge:
	...
	\caption{\protect\small #5 --- [FIXME: Cannot do both rotation and scaling in latex2html graphicx support]}\label{fig:#1}
	\end{figure}
}
...
%prv:	\resizebox{#2}{#3}{\includegraphics{eps/#1}}
	...
	\caption{\protect\small #4}\label{fig:#1}
	\end{figure}
}
...
% \protect\small matches the .aux entry written by the LaTeX-build counterpart
% (\mycaptionparlabel wraps the caption in \protect\small), so latex2html
% caption-matching succeeds and tables get numbers ("Table B.1:" not "Table:").
\newcommand{\myTable}[3]{
	\begin{table}[htbp]
	\begin{center}
	#3
